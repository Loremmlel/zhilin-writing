...
export const posts = sqliteTable(
  ...
  (table) => [
    ...
    uniqueIndex("posts_author_creation_submission_unique").on(
      table.authorId,
      table.creationSubmissionKey,
    ),
  ],
);
...
export const annotations = sqliteTable(
  "annotations",
  {
    ...
    createdOnRevisionId: text("created_on_revision_id")
      .notNull()
    ...
    sourceType: text("source_type", { enum: ["NATIVE", "DOCX_IMPORT"] })
      .notNull()
      .default("NATIVE"),
  ...
  (table) => [
    ...
    index("annotations_post_source_order_idx").on(
      table.postId,
      table.sourceType,
      table.sourceDocumentOrder,
    ),
    index("annotations_attributed_user_idx").on(table.attributedUserId),
    uniqueIndex("annotations_import_source_unique")
      .on(table.importBatchId, table.sourceCommentId)
      .where(sql`${table.sourceType} = 'DOCX_IMPORT'`),
    check("annotations_source_type_check", sql`${table.sourceType} in ('NATIVE', 'DOCX_IMPORT')`),
    check(
      "annotations_anchor_retirement_check",
      sql`
    ...
    check(
      "annotations_source_identity_check",
      sql`
...
export const annotationReplies = sqliteTable(
  ...
  (table) => [
    ...
    uniqueIndex("annotation_replies_author_submission_unique").on(
      table.authorId,
      table.submissionKey,
    ),
    index("annotation_replies_import_batch_idx").on(table.importBatchId),
    index("annotation_replies_annotation_source_order_idx").on(
      table.annotationId,
      table.sourceType,
      table.sourceDocumentOrder,
    ),
    index("annotation_replies_attributed_user_idx").on(table.attributedUserId),
    uniqueIndex("annotation_replies_import_source_unique")
      .on(table.importBatchId, table.sourceCommentId)
      .where(sql`${table.sourceType} = 'DOCX_IMPORT'`),
    check(
      "annotation_replies_source_type_check",
      sql`${table.sourceType} in ('NATIVE', 'DOCX_IMPORT')`,
    ),
    check(
      "annotation_replies_source_identity_check",
      sql`
      (${table.sourceType} = 'NATIVE'
        and ${table.authorId} is not null
        and ${table.sourceAuthorName} is null
...
export const adminAuditLog = sqliteTable(
  "admin_audit_log",
  {
    ...
    targetType: text("target_type", {
      enum: ["POST", "REPLY", "ANNOTATION", "ANNOTATION_REPLY"],
    }).notNull(),
...
export const notifications = sqliteTable(
  ...
  (table) => [
    ...
    uniqueIndex("notifications_event_recipient_type_unique").on(
      table.eventId,
      table.recipientUserId,
      table.notificationType,
    ),
    uniqueIndex("notifications_attribution_batch_unique")
      .on(table.recipientUserId, table.importBatchId, table.notificationType)
      .where(sql`${table.notificationType} = 'DOCX_ATTRIBUTION_NOTICE'`),
    check(
      "notifications_type_check",
      sql`${table.notificationType} in ('POST_REPLY_RECEIVED', 'POST_ANNOTATION_RECEIVED', 'ANNOTATION_REPLY_RECEIVED', 'DOCX_ATTRIBUTION_NOTICE')`,
    ),
    check(
      "notifications_import_batch_check",
      sql`
...
export const assets = sqliteTable(
  "assets",
  {
    ...
    status: text("status", { enum: ["temporary", "permanent"] })
      .notNull()
      .default("temporary"),
    createdAt: integer("created_at", { mode: "timestamp_ms" }).notNull(),
    ...
    gcLastErrorCode: text("gc_last_error_code", {
      enum: ["R2_UNAVAILABLE", "R2_DELETE_FAILED", "METADATA_UPDATE_FAILED"],
    }),
  },
  (table) => [
...
export const postRevisions = sqliteTable(
  "post_revisions",
  {
    ...
    kind: text("kind", { enum: ["CONTENT_EDIT", "RESTORE", "ANNOTATION_STATE"] })
      .notNull()
...
export const importBatches = sqliteTable(
  ...
  {
    id: text("id").primaryKey(),
    importerUserId: text("importer_user_id")
      .notNull()
      .references(() => users.id),
...
export const revisionImportedReplyStates = sqliteTable(
  "revision_imported_reply_states",
  {
    revisionId: text("revision_id")
      .notNull()
      .references(() => postRevisions.id),
    annotationReplyId: text("annotation_reply_id")
      .notNull()
