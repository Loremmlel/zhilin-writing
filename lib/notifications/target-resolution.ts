...
export function resolveNotificationTarget(input: {
  ...
  if (!input.postExists || input.targetState === null) return { kind: input.kind, state: "NOT_FOUND" };
  ...
  if (input.kind !== "POST_REPLY" && !input.annotationCurrent) return { kind: input.kind, state: "NOT_IN_CURRENT_REVISION" };
  if (!input.postReachable) return { kind: input.kind, state: "POST_UNAVAILABLE" };
  return { kind: input.kind, state: "AVAILABLE" };
}

export function notificationTargetNotice(state: Exclude<NotificationTargetState, "AVAILABLE">, kind: NotificationTargetKind): string {
  ...
  if (state === "NOT_IN_CURRENT_REVISION") return "该内容存在于历史版本中，但当前版本已不再包含它。";
  if (state === "POST_UNAVAILABLE") return "关联帖子当前不可用。";
  return "没有找到这条通知关联的内容。";
}

export function notificationNoticeCode(state: Exclude<NotificationTargetState, "AVAILABLE">, kind: NotificationTargetKind): string {
  return `${kind.toLocaleLowerCase("en-US").replaceAll("_", "-")}-${state.toLocaleLowerCase("en-US").replaceAll("_", "-")}`;
}

export function targetHrefWithNotice(href: string, state: Exclude<NotificationTargetState, "AVAILABLE">): string {
...
export function parseNotificationTargetState(value: string | undefined): Exclude<NotificationTargetState, "AVAILABLE"> | null {
