...
async function referenceCounts(assetId: string): Promise<ReferenceCounts> {
  ...
  const [current, revisions, avatars] = await Promise.all([
    db
      .select({ assetId: postAssetRefs.assetId })
      .from(postAssetRefs)
...
export async function collectOrphanedAssets(
      ...
      and(
        ...
        notExists(
          db
            .select({ assetId: postAssetRefs.assetId })
            .from(postAssetRefs)
            .where(eq(postAssetRefs.assetId, assets.id)),
        ),
        notExists(
          db
            .select({ assetId: revisionAssetRefs.assetId })
            .from(revisionAssetRefs)
            .where(eq(revisionAssetRefs.assetId, assets.id)),
        ),
        notExists(
          db.select({ id: users.id }).from(users).where(eq(users.avatarAssetId, assets.id)),
        ),
        or(
          eq(assets.status, "permanent"),
          and(
            eq(assets.status, "temporary"),
            lte(assets.createdAt, temporaryCutoff),
            isNotNull(assets.expiresAt),
  ...
  const report = await runAssetGcCandidates({
    ...
    dependencies: {
      ...
      async releaseClaim(assetId) {
        await db
          .update(assets)
          .set({ gcClaimedAt: null })
          .where(and(eq(assets.id, assetId), eq(assets.gcClaimedAt, now)));
      },
  ...
  report.failures.forEach((failure) =>
    logServerError({ operation: "asset.gc", entityId: failure.assetId, errorCode: failure.code }),
  );
  return report;
}
  ...
  const tracked =
    keys.length === 0
      ...
      : await getDb()
          ...
          .from(assets)
          .where(inArray(assets.r2Key, keys));
  const candidates = untrackedR2Candidates(
    page.objects,
    new Set(tracked.map((asset) => asset.r2Key)),
    now,
  );
  const report = {
    dryRun: options.dryRun ?? true,
  ...
  for (const candidate of candidates) {
    const trackedNow = (
      await getDb()
        .select({ id: assets.id })
        .from(assets)
        .where(eq(assets.r2Key, candidate.key))
        .limit(1)
    )[0];
