...
export function validateAssetUpload(file: { size: number; mimeType: string }): AssetUploadValidationFailure | null {
  ...
  if (mimeType.startsWith("image/") && !supportedImageTypes.has(mimeType)) {
    return { code: "UNSUPPORTED_TYPE", message: "暂不支持此图片格式，请使用 PNG、JPEG、GIF 或 WebP" };
  }
