...
export function createAnnotationGuardPlugin(options: AnnotationGuardPluginOptions) {
      ...
      new Plugin({
        filterTransaction(transaction, state) {
          ...
          if (decision.kind === "REPLACE") {
            queueMicrotask(() => {
              if (
                !editorView ||
                !editorView.state.doc.eq(state.doc) ||
                JSON.stringify(editorView.state.selection.toJSON()) !==
        ...
        props: {
          ...
          transformPasted(slice, view) {
            ...
            return inheritDestinationAnnotationMark(
              stripAnnotationMarksFromSlice(slice, markType),
              view.state,
              markType,
            );
          },
          handleDOMEvents: {
            ...
            beforeinput(_view, event) {
              ...
              if (
                !session.blockCompositionUpdate() ||
                (!input.isComposing && !input.inputType.includes("Composition"))
              )
                return false;
              event.preventDefault();
              return true;
              ...
              if (restoreAnnotationIds.length > 0) {
                window.setTimeout(() => {
                  const present = new Set(
                    analyzeAnnotationRanges(view.state.doc).ranges.map(
                      (range) => range.annotationId,
                    ),
                  );
  ...
  function confirmPending(token: number): AnnotationConfirmationResult | null {
    ...
    if (pendingCut?.token === token) {
      ...
      pendingCut = null;
      const stateMatches =
        cut.beforeDoc.eq(editorView.state.doc) &&
