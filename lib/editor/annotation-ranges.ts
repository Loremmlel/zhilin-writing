...
export type AnnotationRangeIssueCode =
  "EMPTY" | "MULTI_BLOCK" | "DUPLICATE" | "OVERLAP" | "NESTED" | "INVALID_BLOCK";
...
const allowedFormattingMarks = new Set([
  "annotation",
  "strong",
  "emphasis",
  "strike_through",
...
function endpointFor(
  pieces: TextPiece[],
  segment: { index: number; text: string } | undefined,
): EditorAnnotationEndpoint {
  if (!segment) {
  ...
  doc.descendants((node, pos, parent) => {
    if (node.isTextblock) {
      ...
      node.descendants((child, relativePos) => {
        if (child.isText) {
          if (
            !child.marks.every(
              (mark) =>
                allowedFormattingMarks.has(mark.type.name) &&
      ...
      const validContainer =
        (blockType === "heading" && parent?.type.name === "doc") ||
        (blockType === "paragraph" &&
          ["doc", "list_item", "blockquote"].includes(parent?.type.name ?? ""));
      blocks.set(pos, {
        blockFrom: pos,
...
export function analyzeAnnotationRanges(doc: ProseMirrorNode): {
  ...
  for (const builder of builders) {
    if (!builder.validBlock || !builder.validInline)
      addIssue(issues, builder.annotationId, "INVALID_BLOCK");
  ...
  for (const [annotationId, ranges] of buildersById) {
    ...
    for (const range of ranges)
      byBlock.set(range.blockFrom, (byBlock.get(range.blockFrom) ?? 0) + 1);
    if ([...byBlock.values()].some((count) => count > 1))
      addIssue(issues, annotationId, "DUPLICATE");
    else if (
  ...
  for (const current of builders) {
    const active = (activeByBlock.get(current.blockFrom) ?? []).filter(
      (range) => range.to > current.from,
    );
    for (const other of active) {
      if (other.annotationId === current.annotationId || other.to <= current.from) continue;
      const nested =
        (other.from <= current.from && other.to >= current.to) ||
