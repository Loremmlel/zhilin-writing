import { describeAnnotationDomRange, restoreSerializedDomRange, serializeDomRange, type SerializedDomRange } from "./dom-selection.ts";
...
export type SelectionPreviewPhase = "hidden" | "bubble" | "composer" | "pending" | "failed" | "awaiting-annotation";
export type SelectionPreviewEvent = "capture" | "open-composer" | "submit" | "failure" | "retry" | "submit-succeeded" | "annotation-present" | "cancel" | "body-click" | "invalidate" | "revision-change" | "unmount";
...
export function validateSavedAnnotationSelection(saved: SavedAnnotationSelection, current: {
  postId: string;
  baseRevisionId: string;
  root: Element;
  epoch: number;
}): Range | null {
  if (saved.postId !== current.postId || saved.baseRevisionId !== current.baseRevisionId || saved.epoch !== current.epoch) return null;
  ...
  try {
    ...
    return descriptor.selectedText === saved.selectedText && sameDescriptor(descriptor, saved.descriptor) ? range : null;
  } catch {
    return null;
  }
}

export function nextSelectionPreviewPhase(current: SelectionPreviewPhase, event: SelectionPreviewEvent): SelectionPreviewPhase {
  if (["annotation-present", "cancel", "body-click", "invalidate", "revision-change", "unmount"].includes(event)) return "hidden";
...
function ensureHighlightStyle(ownerDocument: Document) {
  ...
  try {
    ...
    style.sheet.insertRule(`::highlight(${highlightName}) { background: var(--annotation-active); text-decoration: underline dotted var(--annotation-ink); text-underline-offset: 3px; }`);
    style.sheet.insertRule(`@media (forced-colors: active) { ::highlight(${highlightName}) { background: Highlight; color: HighlightText; text-decoration-color: HighlightText; } }`);
    return true;
  } catch {
    style.remove();
    return false;
...
export function paintAnnotationSelectionPreview(range: Range): () => void {
  ...
  const cssHighlights = (globalThis.CSS as typeof CSS & { highlights?: HighlightRegistry } | undefined)?.highlights;
  ...
  const draw = () => {
    ...
    overlay.replaceChildren(...rectangles.map((rectangle) => {
