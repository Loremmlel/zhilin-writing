...
export function isSupportedAnnotationBlock(node: MarkdownNodeLike, parentType: string): boolean {
  return (
    (node.type === "heading" && parentType === "root") ||
    (node.type === "paragraph" && ["root", "listItem", "blockquote"].includes(parentType))
  );
}

export function collectAnnotationDocumentBlocks<T extends MarkdownNodeLike>(
  root: T,
): AnnotationDocumentBlock<T>[] {
...
export function isCanonicalMultiBlockAnnotationSpan(
  segments: readonly AnnotationSpanSegment[],
): boolean {
  if (segments.length <= 1) return true;
  const ordered = [...segments].sort(
    (left, right) => left.blockIndex - right.blockIndex || left.from - right.from,
  );
  if (
    ordered.some(
      (segment, index) => index > 0 && segment.blockIndex !== ordered[index - 1]!.blockIndex + 1,
    )
  )
    return false;
