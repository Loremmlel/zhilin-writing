...
export function planAnnotationCreation(
  current: CurrentAnnotationPostState,
  request: {
  ...
  if (visiblePostText(currentTree) !== visiblePostText(nextTree))
    throw new Error("创建批注不能改变正文可见文字");
  const nextAnchorIds = collectAnnotationIds(nextTree);
  const markdown = stringifyAnnotationMarkdown(nextTree);
  if (
    !validateCanonicalAnnotationDocument(markdown, [...current.currentAnchorIds, annotationId]).ok
  ) {
    throw new Error("正文批注锚点状态不一致");
  }
  return {
    ...
    revisionKind: "ANNOTATION_STATE" as const,
    revisionNumber: current.currentRevisionNumber + 1,
    editedAt: current.editedAt,
    lastActivityAt: now,
    originalSelectedText: request.selection.selectedText,
    previousAnchorIds: [...current.currentAnchorIds],
    anchorIds: nextAnchorIds,
    notificationRecipientUserId:
      current.actorUserId === current.postAuthorId ? null : current.postAuthorId,
  };
}
...
export function planAnnotationReplyCreation(
  annotation: {
    id: string;
    postId: string;
    authorId: string | null;
  ...
  input: { actorUserId: string; targetReply: ReplyTarget | null },
  now: Date,
) {
  ...
  if (input.targetReply) {
    ...
    if (input.targetReply.deletedAt || input.targetReply.hiddenAt)
  ...
  return {
    ...
    replyToReplyId: input.targetReply?.id ?? null,
    replyToUserId,
    notificationRecipientUserId: resolveAnnotationReplyRecipient({
      actorUserId: input.actorUserId,
      annotationAuthorId: annotation.authorId,
      replyToUserId,
    }),
    visualDepth: 1 as const,
    lastActivityAt: now,
  };
}
