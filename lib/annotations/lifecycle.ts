...
export function buildAnnotationReplyLifecycleViews<T extends AnnotationReplyLifecycleInput>(
  ...
  return replies.flatMap((reply) => {
    ...
    const dependents = replies.filter(
      (candidate) =>
        candidate.replyToReplyId === reply.id && contentState(candidate).state === "normal",
    );
    if (state !== "normal" && dependents.length === 0 && !requiredPlaceholderIds.has(reply.id))
      return [];
    return [
      {
        ...reply,
        ...
        visibleOtherAuthorDependentCount: dependents.filter(
          (candidate) => candidate.authorId !== reply.authorId,
        ).length,
      },
    ];
  });
...
export function planImportedAnnotationThreadRemoval(
  ...
  if (
    context.actorUserId !== annotation.importedByUserId &&
    context.actorUserId !== context.postAuthorId
  )
    throw new Error("你不能移除这条 Word 导入批注");
  if (annotation.deletedAt)
    return {
      changed: false as const,
      retainAnchor: false,
      importedReplyIds: [] as string[],
      patch: {},
    };
  return {
    changed: true as const,
    retainAnchor: replies.some((reply) => reply.sourceType === "NATIVE" && !reply.deletedAt),
    importedReplyIds: replies
      .filter((reply) => reply.sourceType === "DOCX_IMPORT" && !reply.deletedAt)
      .map((reply) => reply.id),
...
export function planAnnotationAdminTransition(input: {
  targetType: "ANNOTATION" | "ANNOTATION_REPLY";
  targetId: string;
  record: AdminTarget;
  ...
  if (!changed)
    return {
      ...
      patch: {},
      audit: null,
      createAnnotationStateRevision: false,
    };
  const actionType:
    | "ANNOTATION_HIDDEN"
    | "ANNOTATION_UNHIDDEN"
  ...
  const patch = hiding
    ? {
        hiddenAt: input.now,
        hiddenByUserId: input.administratorId,
        hiddenReason: input.reason?.trim() || null,
      }
  ...
  return {
    ...
    audit: {
      ...
      createdAt: input.now,
      metadataJson: JSON.stringify({ reason: input.reason?.trim() || null }),
