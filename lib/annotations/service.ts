...
import {
  findAnnotation,
  findAnnotationBySubmissionKey,
  findAnnotationReply,
  findAnnotationReplyBySubmissionKey,
  getCurrentAnnotationAnchorIds,
  getCurrentAnnotationStates,
} from "./queries";
...
import {
  assertAnnotationBelongsToPost,
  assertNativeAnnotationMutation,
  createAnnotationId,
  validateAnnotationContent,
  validateAnnotationReplyContent,
  validateAnnotationSubmissionKey,
} from "./policy";
import { unwrapAnnotation } from "./selection";
import {
  buildImportedThreadRemovalPostGuard,
  commitAnnotationMutation,
  planAnnotationCreation,
  planAnnotationReplyCreation,
} from "./transaction";
...
export async function createAnnotation(input: {
  ...
  selection: AnnotationSelectionDescriptor;
  ...
  const currentRevision = (
    await db
      ...
      .where(
        and(
          eq(postRevisions.id, post.post.currentRevisionId),
  ...
  const plan = planAnnotationCreation(
    {
      ...
      editedAt: post.post.editedAt,
      lastActivityAt: post.post.lastActivityAt,
      currentAnchorIds: currentStates.map((state) => state.annotationId),
    },
    { baseRevisionId: input.baseRevisionId, annotationId, revisionId, selection: input.selection },
    now,
  );
  ...
  const operations: BatchItem<"sqlite">[] = [
    db
      .update(posts)
      .set({
        title: sql<string>`CASE WHEN ${posts.currentRevisionId} = ${input.baseRevisionId} AND ${posts.deletedAt} IS NULL AND ${posts.hiddenAt} IS NULL THEN ${posts.title} ELSE NULL END`,
      })
    ...
    db.insert(annotations).values({
      ...
      contentMarkdown,
      originalSelectedText: plan.originalSelectedText,
      createdAt: now,
      createdOnRevisionId: revisionId,
      submissionKey,
      deletedAt: null,
    ...
    db
      ...
      .where(and(eq(posts.id, input.postId), eq(posts.currentRevisionId, input.baseRevisionId))),
    ...
    db.insert(revisionAnnotationStates).values({
      revisionId,
      annotationId,
      deletedAt: null,
      deletedByUserId: null,
    ...
    db.insert(activityEvents).values({
      ...
      metadataJson: JSON.stringify({
        title: post.post.title,
        selectedText: truncateActivityPreview(plan.originalSelectedText, 48),
      }),
      createdAt: now,
      invalidatedAt: null,
    }),
  ];
  if (plan.notificationRecipientUserId)
    operations.push(
      db.insert(notifications).values({
        id: notificationId(eventId, plan.notificationRecipientUserId, "POST_ANNOTATION_RECEIVED"),
        recipientUserId: plan.notificationRecipientUserId,
        actorUserId: input.authorId,
        eventId,
        notificationType: "POST_ANNOTATION_RECEIVED",
        postId: input.postId,
        replyId: null,
        annotationId,
        annotationReplyId: null,
  ...
  try {
    ...
    const latest = (
      await db
        .select({
          currentRevisionId: posts.currentRevisionId,
    ...
    if (latest?.currentRevisionId !== input.baseRevisionId)
      throw new Error("帖子已更新，请重新选择文字后再批注");
    throw error;
  }
  return annotationId;
}
...
export async function createAnnotationReply(input: {
  ...
  const [annotation, post, currentAnchorIds, targetReply] = await Promise.all([
    findAnnotation(input.annotationId),
    getPost(input.postId),
    getCurrentAnnotationAnchorIds(input.postId),
    input.targetReplyId ? findAnnotationReply(input.targetReplyId) : Promise.resolve(null),
  ]);
  if (!post) throw new Error("帖子不存在或当前不可访问");
  if (!annotation || annotation.postId !== input.postId)
    throw new Error("批注不存在或不属于当前帖子");
  if (!post.post.currentRevisionId) throw new Error("帖子当前版本不存在");
  const now = new Date();
  const plan = planAnnotationReplyCreation(
    {
      id: annotation.id,
      postId: annotation.postId,
      authorId: annotation.authorId,
      hiddenAt: annotation.hiddenAt,
      currentAnchorIds,
    },
    { actorUserId: input.authorId, targetReply },
    now,
  );
  ...
  const operations: BatchItem<"sqlite">[] = [
    db
      .update(posts)
      .set({
        title: sql<string>`CASE WHEN ${posts.currentRevisionId} = ${post.post.currentRevisionId} AND ${posts.deletedAt} IS NULL AND ${posts.hiddenAt} IS NULL THEN ${posts.title} ELSE NULL END`,
      })
      .where(eq(posts.id, input.postId)),
    ...(targetReply
      ? [
          db
            ...
            .set({
              contentMarkdown: sql<string>`CASE WHEN ${annotationReplies.annotationId} = ${annotation.id} AND ${annotationReplies.deletedAt} IS NULL AND ${annotationReplies.hiddenAt} IS NULL THEN ${annotationReplies.contentMarkdown} ELSE NULL END`,
            })
            .where(eq(annotationReplies.id, targetReply.id)),
        ]
      : []),
    db.insert(annotationReplies).values({
      id,
      annotationId: annotation.id,
      authorId: input.authorId,
      replyToUserId: plan.replyToUserId,
      replyToReplyId: plan.replyToReplyId,
      contentMarkdown,
      submissionKey,
      createdAt: now,
      deletedAt: null,
    ...
    db.insert(activityEvents).values({
      ...
      replyId: null,
      annotationId: annotation.id,
      annotationReplyId: id,
      rootReplyId: null,
      replyToUserId: plan.replyToUserId,
      metadataJson: JSON.stringify({ title: post.post.title }),
      createdAt: now,
      invalidatedAt: null,
    }),
  ];
  if (plan.notificationRecipientUserId)
    operations.push(
      db.insert(notifications).values({
        id: notificationId(eventId, plan.notificationRecipientUserId, "ANNOTATION_REPLY_RECEIVED"),
        recipientUserId: plan.notificationRecipientUserId,
        actorUserId: input.authorId,
        eventId,
        notificationType: "ANNOTATION_REPLY_RECEIVED",
        postId: input.postId,
        replyId: null,
        annotationId: annotation.id,
        annotationReplyId: id,
  ...
  try {
    ...
    if (!latestPost || latestPost.deletedAt || latestPost.hiddenAt)
      throw new Error("帖子不存在或当前不可访问");
    if (latestPost.currentRevisionId !== post.post.currentRevisionId)
      throw new Error("帖子已更新，请重新打开批注讨论后再回复");
    if (targetReply) {
      const latestTarget = await findAnnotationReply(targetReply.id);
      if (
        !latestTarget ||
        latestTarget.annotationId !== annotation.id ||
        latestTarget.deletedAt ||
        latestTarget.hiddenAt
      )
        throw new Error("该回复当前不可回复");
    }
    throw error;
  }
  return id;
...
export async function deleteAnnotationByAuthor(
  postId: string,
  annotationId: string,
  actorUserId: string,
) {
...
export async function removeImportedAnnotationThread(
  ...
  const currentRevision = (
    await db
      .select()
      .from(postRevisions)
      .where(and(eq(postRevisions.id, post.currentRevisionId), eq(postRevisions.postId, postId)))
      .limit(1)
  )[0];
  if (!currentRevision) throw new Error("帖子当前版本不存在");
  const [currentStates, currentAssetRefs, currentImportedReplyStates] = await Promise.all([
  ...
  const nextStates = currentStates
    .filter((state) => lifecyclePlan.retainAnchor || state.annotationId !== annotationId)
    .map((state) =>
      state.annotationId === annotationId ? { ...state, ...lifecyclePlan.patch } : state,
    );
  assertCanonicalAnchors(
    nextMarkdown,
    nextStates.map((state) => state.annotationId),
  ...
  const removedReplyIds = new Set(
    discussionReplies
      .filter((reply) => reply.sourceType === "DOCX_IMPORT")
      .map((reply) => reply.id),
  );
  const nextImportedReplyStates = currentImportedReplyStates.map((state) =>
    removedReplyIds.has(state.annotationReplyId) ? { ...state, ...lifecyclePlan.patch } : state,
  );
  ...
  const operations: BatchItem<"sqlite">[] = [
    db
      .update(posts)
      .set({ title: sql<string>`CASE WHEN ${postGuard} THEN ${posts.title} ELSE NULL END` })
      .where(eq(posts.id, postId)),
    db.insert(postRevisions).values({
      id: revisionId,
      postId,
    ...
    db
      ...
      .where(
        and(
          eq(annotations.id, annotationId),
          eq(annotations.sourceType, "DOCX_IMPORT"),
          isNull(annotations.deletedAt),
        ),
      ),
    db
      ...
      .where(
        and(
          ...
          eq(annotationReplies.sourceType, "DOCX_IMPORT"),
          isNull(annotationReplies.deletedAt),
        ),
      ),
    db
      .update(posts)
      .set({ markdown: nextMarkdown, currentRevisionId: revisionId, lastActivityAt })
      .where(and(eq(posts.id, postId), eq(posts.currentRevisionId, currentRevision.id))),
    ...(!lifecyclePlan.retainAnchor
      ? [
          db
            .delete(postAnnotationAnchors)
    ...
    ...nextImportedReplyStates.map((state) =>
      db.insert(revisionImportedReplyStates).values({ revisionId, ...state }),
    ),
  ];
  await commitAnnotationMutation((items) => db.batch(asBatch(items)), operations);
...
export async function deleteAnnotationReplyByAuthor(
  postId: string,
  replyId: string,
  actorUserId: string,
) {
  ...
  const lastActivityAt = await derivePostActivityAfterInteractionChange(annotation.postId, {
    kind: "annotationReply",
    id: replyId,
    deletedAt: plan.patch.deletedAt,
  });
  const db = getDb();
  await commitAnnotationMutation(
    (items) => db.batch(asBatch(items)),
    [
      db
        .update(annotationReplies)
        .set(plan.patch)
        .where(
          and(
            eq(annotationReplies.id, replyId),
            eq(annotationReplies.sourceType, "NATIVE"),
            eq(annotationReplies.authorId, actorUserId),
            isNull(annotationReplies.deletedAt),
          ),
        ),
...
export async function moderateAnnotationByAdmin(input: {
  annotationId: string;
  ...
  const plan = planAnnotationAdminTransition({
  ...
  const guard =
    input.operation === "hide" ? isNull(annotations.hiddenAt) : isNotNull(annotations.hiddenAt);
  const auditInsert = db
    .insert(adminAuditLog)
    .values({ id: crypto.randomUUID(), ...plan.audit })
    ...
    await commitAnnotationMutation(
      ...
      [
        db
          .update(annotations)
          .set(plan.patch)
          .where(and(eq(annotations.id, annotation.id), guard)),
        auditInsert,
      ],
    );
    return { changed: true as const, postId: annotation.postId };
  ...
  const nextStates = currentStates.map((state) =>
    state.annotationId === annotation.id ? { ...state, ...plan.patch } : state,
  );
  await commitAnnotationMutation(
    (items) => db.batch(asBatch(items)),
    [
      ...
      db.insert(postRevisions).values({
        ...
        createdAt: plan.audit.createdAt,
        createdByUserId: input.administratorId,
        restoreSourceRevisionId: null,
      }),
      db
        .update(annotations)
...
export async function moderateAnnotationReplyByAdmin(input: {
  replyId: string;
  administratorId: string;
  operation: "hide" | "unhide";
  operationId: string;
  ...
  const plan = planAnnotationAdminTransition({
    ...
    operationId: input.operationId,
    reason: input.reason,
    currentAnchor: currentAnchorIds.includes(annotation.id),
    now: new Date(),
  });
  if (!plan.changed || !plan.audit) return { changed: false as const, postId: annotation.postId };
  const db = getDb();
  const guard =
    input.operation === "hide"
      ? isNull(annotationReplies.hiddenAt)
  ...
  await commitAnnotationMutation(
    ...
    [
      db
        .update(annotationReplies)
        .set(plan.patch)
        .where(and(eq(annotationReplies.id, reply.id), guard)),
      db
        .insert(adminAuditLog)
        .values({ id: crypto.randomUUID(), ...plan.audit })
        .onConflictDoNothing({ target: adminAuditLog.dedupeKey }),
