...
export async function listCurrentAnnotations(postId: string) {
  const rows = await getDb().select({ annotation: annotations, nativeAuthor: annotationAuthor, attributedUser: annotationAttributedUser }).from(postAnnotationAnchors)
    ...
    .leftJoin(annotationAttributedUser, eq(annotations.attributedUserId, annotationAttributedUser.id))
...
export async function listCurrentAnnotationThreads(postId: string, options: { includeUnavailableReplyId?: string } = {}) {
  ...
  const rawReplyRows = await getDb().select({ reply: annotationReplies, nativeAuthor: replyAuthor, attributedUser: replyAttributedUser }).from(annotationReplies)
  ...
  const replyRows = sortAnnotationReplyRows(rawReplyRows.map(({ reply, nativeAuthor, attributedUser }) => ({
    reply,
  ...
  return roots.map((root) => {
    ...
    return {
      ...
      replies: threadRows.flatMap((row) => {
        ...
        if (!view) return [];
        const replyTarget = row.reply.replyToReplyId ? replyById.get(row.reply.replyToReplyId)?.author ?? null : null;
        const userTarget = row.reply.replyToUserId ? targetById.get(row.reply.replyToUserId) ?? null : null;
        return [{ ...row, reply: view.contentVisible ? row.reply : { ...row.reply, contentMarkdown: "" }, lifecycle: view, replyTo: replyTarget ?? userTarget }];
      }),
    };
  });
}
...
export async function listAdminAnnotations(status: AnnotationAdminStatus, limit = 100) {
  ...
  const rows = await getDb().select({ annotation: annotations, nativeAuthor: annotationAuthor, attributedUser: annotationAttributedUser, post: posts }).from(annotations)
    .leftJoin(annotationAuthor, eq(annotations.authorId, annotationAuthor.id))
    .leftJoin(annotationAttributedUser, eq(annotations.attributedUserId, annotationAttributedUser.id))
    .innerJoin(posts, eq(annotations.postId, posts.id))
    .where(condition).orderBy(desc(annotations.createdAt)).limit(limit);
  return rows.map(({ annotation, nativeAuthor, attributedUser, post }) => ({ annotation, author: buildAnnotationAuthorView(annotation, nativeAuthor, attributedUser), post }));
}
...
export async function listAdminAnnotationReplies(status: AnnotationAdminStatus, limit = 100) {
  ...
  const rows = await getDb().select({ reply: annotationReplies, nativeAuthor: replyAuthor, attributedUser: replyAttributedUser, annotation: annotations, post: posts }).from(annotationReplies)
    .leftJoin(replyAuthor, eq(annotationReplies.authorId, replyAuthor.id))
    .leftJoin(replyAttributedUser, eq(annotationReplies.attributedUserId, replyAttributedUser.id))
    .innerJoin(annotations, eq(annotationReplies.annotationId, annotations.id)).innerJoin(posts, eq(annotations.postId, posts.id))
    .where(condition).orderBy(desc(annotationReplies.createdAt)).limit(limit);
  return rows.map(({ reply, nativeAuthor, attributedUser, annotation, post }) => ({ reply, author: buildAnnotationAuthorView(reply, nativeAuthor, attributedUser), annotation, post }));
}
