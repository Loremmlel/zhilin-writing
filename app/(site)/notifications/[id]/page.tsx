...
export default async function OpenNotificationPage({
  ...
  if (
    item.notification.notificationType === "DOCX_ATTRIBUTION_NOTICE" &&
    item.postReachable &&
    item.post
  ) {
    return (
      <div className="page-column notifications-page">
        <div className="unavailable-card">
          ...
          <p>
            {item.docxAttribution
              ...
              : "这条 DOCX 批注关联通知的数据不完整，无法显示原始汇总。"}
          </p>
          <Link
            className="button button--primary"
            href={`/posts/${encodeURIComponent(item.post.id)}`}
          >
            查看帖子
          </Link>
        </div>
  ...
  if (!item.post || resolution.state === "POST_UNAVAILABLE" || resolution.state === "NOT_FOUND") {
    ...
    return (
      <div className="page-column">
        <div className="unavailable-card">
          <span className="eyebrow">历史通知</span>
          <h1>无法打开通知目标</h1>
          <p>{copy}</p>
        </div>
      </div>
  ...
  const targetHref =
    resolution.kind === "POST_REPLY" && item.notification.replyId
      ...
      : resolution.kind === "ANNOTATION" && item.notification.annotationId
        ...
        : resolution.kind === "ANNOTATION_REPLY" &&
            item.notification.annotationId &&
            item.notification.annotationReplyId
          ? annotationReplyTargetHref(
              item.post.id,
              item.notification.annotationId,
              item.notification.annotationReplyId,
  ...
  redirect(
    resolution.state === "AVAILABLE"
      ? targetHref
      : targetHrefWithNotice(targetHref, resolution.state),
  );
}
