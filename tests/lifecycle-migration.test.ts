...
test("V4 migration preserves V3 data and backfills direct reply targets", async () => {
  ...
  for (const [id, name] of [["author", "作者"], ["member-b", "乙"], ["member-c", "丙"]]) {
  ...
  db.prepare("INSERT INTO replies (id, post_id, author_id, root_reply_id, reply_to_user_id, submission_key, markdown, published_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
  ...
  assert.deepEqual(
    db.prepare("SELECT id, reply_to_reply_id FROM replies ORDER BY published_at").all().map((row) => ({ ...row })),
    [
  ...
  assert.deepEqual({ ...db.prepare("SELECT deleted_by_user_id, hidden_by_user_id, hidden_reason FROM posts WHERE id = 'post-1'").get() }, {
    deleted_by_user_id: null,
    hidden_by_user_id: null,
    hidden_reason: null,
  });
  assert.deepEqual({ ...db.prepare("SELECT deleted_by_user_id, hidden_by_user_id, hidden_reason FROM replies WHERE id = 'reply-a'").get() }, {
    deleted_by_user_id: null,
    hidden_by_user_id: null,
    hidden_reason: null,
  });

  db.prepare("INSERT INTO admin_audit_log (id, admin_user_id, action_type, target_type, target_id, created_at, metadata_json, dedupe_key) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
    .run("audit-1", "author", "POST_HIDDEN", "POST", "post-1", 600, null, "POST_HIDDEN:POST:post-1:active");
  assert.throws(() => db.prepare("INSERT INTO admin_audit_log (id, admin_user_id, action_type, target_type, target_id, created_at, metadata_json, dedupe_key) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
    .run("audit-2", "author", "POST_HIDDEN", "POST", "post-1", 601, null, "POST_HIDDEN:POST:post-1:active"));
});
