...
test("connectors stay in the gutter and simplify when the gutter is too narrow", () => {
  ...
  assert.equal(annotationConnectorPath({ startX: 740, startY: 90, endX: 755, endY: 150 }), "M 740 90 L 755 150");
});
...
test("unchanged card and connector geometry can preserve React state identity", () => {
  ...
  assert.equal(sameAnnotationConnectors([{ annotationId: "a", path: "M 0 0 L 1 1" }], [{ annotationId: "a", path: "M 0 0 L 1 1" }]), true);
  assert.equal(sameAnnotationConnectors([{ annotationId: "a", path: "M 0 0 L 1 1" }], [{ annotationId: "b", path: "M 0 0 L 1 1" }]), false);
});
