...
test("author delete is owner-only and an already deleted retry is a no-op", () => {
  ...
  assert.deepEqual(
    planAuthorDelete(
      { ...active, deletedAt: now, deletedByUserId: "author" },
      "author",
      new Date(11_000),
    ),
    {
...
test("restore and unhide clear only their own lifecycle state", () => {
  ...
  const restored = planAdminLifecycleTransition(
    "POST_RESTORED",
    "POST",
    "post-1",
    both,
  ...
  const unhidden = planAdminLifecycleTransition(
    "POST_UNHIDDEN",
    "POST",
    "post-1",
    both,
...
test("hide retries are idempotent and retain an earlier author deletion", () => {
  ...
  const first = planAdminLifecycleTransition(
    "REPLY_HIDDEN",
    "REPLY",
    "reply-1",
    deleted,
  ...
  const retry = planAdminLifecycleTransition(
    "REPLY_HIDDEN",
    "REPLY",
    "reply-1",
    {
...
test("administrator operation IDs dedupe retries without collapsing later moderation cycles", () => {
  ...
  const later = planAdminLifecycleTransition(
    "POST_HIDDEN",
    "POST",
    "post-1",
    active,
