...
import {
  buildWordThreads,
  parseWordComments,
} from "../lib/docx-import/annotations.ts";
...
const parseOptions = {
  ...
  createReplyId: (sourceCommentId: string) => `00000000-0000-4000-9000-${sourceCommentId.padStart(12, "0")}`,
};
...
test("keeps adjacent UTF-16 ranges and greedily skips an intersecting candidate", async () => {
  ...
  const parsed = await parseDocx(await commentFixture(body, [
    comment("1", "00000001", "甲", "J", "第一条"),
    comment("2", "00000002", "乙", "Y", "第二条"),
    comment("3", "00000003", "丙", "B", "第三条"),
  ].join(""), undefined, {
    documentTransform: (document) => document.replace(
...
test("forms UTF-16 ranges from decoded predefined and numeric XML entities", async () => {
  ...
  for (const item of cases) {
    ...
    assert.equal("segments" in block ? block.segments.map((segment) => segment.text).join("") : "", `${item.decoded}B`);
    assert.deepEqual([
      parsed.threads[0]?.blockLocalStart,
      parsed.threads[0]?.blockLocalEnd,
...
test("accepts the longest nested range first and skips the inner thread", async () => {
  const parsed = await parseDocx(await commentFixture(`<w:p>
  ...
  ].join("")), undefined, parseOptions);

  assert.deepEqual(parsed.threads.map((thread) => [thread.sourceCommentId, thread.blockLocalStart, thread.blockLocalEnd]), [
    ["10", 0, 4],
  ]);
...
test("builds immediate reply parents from the last comment paragraph and flattens without commentsExtended", () => {
  ...
  const catalog = parseWordComments(xml(definitions, "comments"), xml(extended, "commentsExtended"));
  const threaded = buildWordThreads(catalog);
  assert.deepEqual(threaded.map((thread) => ({
    ...
    rootBody: "第一段\n\n根批注",
    resolved: true,
    replies: [["22", "21", true], ["21", "20", false]],
  }]);

  const flat = buildWordThreads(parseWordComments(xml(definitions, "comments"), undefined));
  assert.deepEqual(flat.map((thread) => [thread.root.sourceCommentId, thread.replies.length]), [
    ["20", 0], ["22", 0], ["21", 0],
  ]);
});
...
test("marks missing-parent duplicate-paraId and cycle components as atomic invalid threads", () => {
  ...
  const threads = buildWordThreads(parseWordComments(xml(definitions, "comments"), xml(extended, "commentsExtended")));
  assert.deepEqual(threads.map((thread) => [
...
test("imports resolved roots and immediate reply metadata through parseDocx", async () => {
  ...
  assert.deepEqual(parsed.threads[0]?.replies.map((reply) => ({
    id: reply.replyId,
