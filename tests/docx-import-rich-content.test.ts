...
const parseOptions = {
  createAnnotationId: (sourceCommentId: string) => `ann_00000000-0000-4000-8000-${sourceCommentId.padStart(12, "0")}`,
  createReplyId: (sourceCommentId: string) => `00000000-0000-4000-8000-${sourceCommentId.padStart(12, "0")}`,
};
...
test("renders rectangular tables and flattens merged tables without raw HTML", async () => {
  ...
  const syntheticHeader = tableXml([
    rowXml([cellXml("无标题 A"), cellXml("无标题 B")]),
  ]);
  ...
  const parsed = await parseDocx(await makeDocxFixture({
    "[Content_Types].xml": MINIMAL_CONTENT_TYPES,
    "word/document.xml": wordDocumentXml(`${explicitHeader}${syntheticHeader}${merged}${gridOffsets}`),
  ...
  assert.match(parsed.canonicalMarkdown, /\| \*\*正文 A\*\* \/ \[第二段\]\(https:\/\/example\.com\/a\\\|b\) \| 正文 B \|/);
  assert.match(parsed.canonicalMarkdown, /\|\s*\|\s*\|\n\| --- \| --- \|\n\| 无标题 A \| 无标题 B \|/);
...
test("flattens every supported Word merged-cell marker", async () => {
  for (const marker of [
    ...
    const parsed = await parseDocx(await makeDocxFixture({
      "[Content_Types].xml": MINIMAL_CONTENT_TYPES,
      "word/document.xml": wordDocumentXml(tableXml([
        rowXml([cellXml("左", undefined, marker), cellXml("右")]),
      ])),
    }), undefined, parseOptions);
    assert.deepEqual(parsed.blocks.map((block) => block.type), ["paragraph"]);
...
test("imports supported embedded images with deterministic alt text and floating degradation", async () => {
  ...
  ].map((drawing) => `<w:p><w:r>${drawing}</w:r></w:p>`).join("");
  const parsed = await parseDocx(await makeDocxFixture({
  ...
  assert.deepEqual(parsed.assets.map((asset) => [asset.mimeType, asset.alt, asset.floating]), [
...
test("rejects image count and per-image byte limits before extraction", async () => {
  const repeated = Array.from({ length: DOCX_IMPORT_LIMITS.images + 1 }, () =>
    `<w:p><w:r>${drawingXml("rImage", false)}</w:r></w:p>`).join("");
  ...
  await assert.rejects(() => parseDocx(tooMany), (error: unknown) =>
    error instanceof DocxImportError && error.code === "IMAGE_COUNT_LIMIT");
  ...
  await assert.rejects(() => parseDocx(oversized), (error: unknown) =>
    error instanceof DocxImportError && error.code === "IMAGE_SIZE_LIMIT");
});
...
test("keeps an image between the paragraph text that surrounds its anchor", async () => {
  ...
  assert.deepEqual(parsed.blocks.map((block) => block.type), ["paragraph", "image", "paragraph"]);
  assert.equal(parsed.threads[0]?.blockId, parsed.blocks[2]?.id);
  assert.match(parsed.canonicalMarkdown, /^图片前\n\n!\[ordered\\.png\]\([^\n]+\)\n\n:annotation\[图片后\]/);
});
...
test("keeps a list item whole when it contains an embedded image", async () => {
  ...
  const parsed = await parseDocx(await makeDocxFixture({
    "[Content_Types].xml": RICH_CONTENT_TYPES,
    "word/document.xml": wordDocumentXml(`<w:p><w:pPr><w:numPr><w:ilvl w:val="0"/><w:numId w:val="1"/></w:numPr></w:pPr>
  ...
  assert.deepEqual(parsed.blocks.map((block) => block.type), ["list", "image"]);
...
test("keeps an empty range before a pure-image anchor classified as empty", async () => {
  const parsed = await parseDocx(await makeDocxFixture({
    "[Content_Types].xml": RICH_CONTENT_TYPES,
    "word/document.xml": wordDocumentXml(`<w:p><w:commentRangeStart w:id="93"/><w:commentRangeEnd w:id="93"/>
...
test("numbers footnotes and endnotes in one appendix and rejects note comments as non-text", async () => {
  ...
  const parsed = await parseDocx(await makeDocxFixture({
    "[Content_Types].xml": MINIMAL_CONTENT_TYPES,
    "word/document.xml": wordDocumentXml(body),
    "word/footnotes.xml": footnotes.replace("</w:footnotes>",
  ...
  assert.match(parsed.canonicalMarkdown, /---\n\n脚注（从 Word 导入）\n\n\[1\] 脚注正文\n\n\[2\] 尾注正文$/);
  assert.equal(warningCount(parsed, "NOTES_FLATTENED_TO_APPENDIX"), 1);
  assert.equal(parsed.threads.length, 0);
  assert.deepEqual(parsed.skippedThreads.map((thread) => [thread.sourceCommentId, thread.warning.code]), [
