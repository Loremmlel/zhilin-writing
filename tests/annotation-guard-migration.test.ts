...
async function createV5Database() {
  ...
  const files = (await readdir(new URL("../drizzle/", import.meta.url)))
    ...
    .sort();
  assert.ok(files.some((name) => name.startsWith("0006_")), "0006 AnnotationGuard migration must exist");
...
function seedAnnotation(db: DatabaseSync) {
  ...
  db.prepare("INSERT INTO post_revisions (id, post_id, revision_number, kind, title, markdown, created_at, created_by_user_id, restore_source_revision_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULL)")
  ...
  db.prepare("INSERT INTO annotations (id, post_id, author_id, content_markdown, original_selected_text, created_at, created_on_revision_id, submission_key) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
...
test("V6 migration adds nullable anchor-retirement lifecycle without changing existing annotations", async () => {
  ...
  try {
    seedAnnotation(db);
    db.prepare("INSERT INTO post_annotation_anchors (post_id, annotation_id) VALUES (?, ?)").run("post", "annotation");
    await applyV6Migration(db);
    assert.deepEqual({ ...db.prepare("SELECT anchor_retired_at, anchor_retired_by_user_id, anchor_retired_reason, deleted_at FROM annotations WHERE id = ?").get("annotation") }, {
      anchor_retired_at: null,
      anchor_retired_by_user_id: null,
      anchor_retired_reason: null,
      deleted_at: null,
    });
    assert.deepEqual({ ...db.prepare("SELECT post_id, annotation_id FROM post_annotation_anchors WHERE annotation_id = ?").get("annotation") }, {
      post_id: "post",
      annotation_id: "annotation",
    });
    db.prepare("UPDATE annotations SET anchor_retired_at = ?, anchor_retired_by_user_id = ?, anchor_retired_reason = ? WHERE id = ?")
...
test("V6 migration rejects incomplete or unknown retirement lifecycle", async () => {
  ...
  try {
    seedAnnotation(db);
    await applyV6Migration(db);
    assert.throws(() => db.prepare("UPDATE annotations SET anchor_retired_reason = ? WHERE id = ?").run("POST_EDIT", "annotation"));
    assert.throws(() => db.prepare("UPDATE annotations SET anchor_retired_at = ?, anchor_retired_by_user_id = ?, anchor_retired_reason = ? WHERE id = ?")
