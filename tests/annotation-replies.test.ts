...
test("annotation replies stay one visual layer and notify the actual target", () => {
  const plan = planAnnotationReplyCreation(
    { id: "a", postId: "p", authorId: "root", hiddenAt: null, currentAnchorIds: ["a"] },
    {
      actorUserId: "c",
      targetReply: {
        id: "b-reply",
  ...
  assert.throws(
    () =>
      planAnnotationReplyCreation(
        { ...root, currentAnchorIds: [] },
        { actorUserId: "b", targetReply: null },
        new Date(),
      ),
    /不属于当前正文/,
  );
...
test("replying to imported content creates no native recipient unless a native reply is targeted", () => {
  ...
  const rootReply = planAnnotationReplyCreation(
    importedRoot,
    { actorUserId: "member", targetReply: null },
    new Date(),
  );
  assert.equal(rootReply.replyToUserId, null);
  assert.equal(rootReply.notificationRecipientUserId, null);

  const importedReply = planAnnotationReplyCreation(
    importedRoot,
    {
      actorUserId: "member",
      targetReply: {
  ...
  const nativeReply = planAnnotationReplyCreation(
    importedRoot,
    {
      actorUserId: "member",
      targetReply: {
...
test("reply 37 retargets the single composer and only success clears its draft", () => {
  ...
  state = nextAnnotationReplyComposerState(state, {
    type: "reply",
    replyId: "reply-37",
    displayName: "林柚子",
  });
