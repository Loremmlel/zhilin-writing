...
test("ordinary internal IME composition stays quiet and undo restores its text", () => {
  const emitted = [];
  const session = createAnnotationGuardSession({ baseAnnotationIds: ["a"], onPendingImpact: (pending) => emitted.push(pending) });
  let state = editorState([{ text: "我喜欢你", marks: [annotationMark("a")] }], { from: 2 }, [history()]);
  assert.deepEqual(session.beginComposition(state), { kind: "ALLOW" });
  const transaction = state.tr.insertText("非常").setMeta("composition", 1);
  assert.equal(session.inspectTransaction(state, transaction, { source: "composition" }).kind, "ALLOW");
  ...
  assert.equal(undo(state, (next) => { undoTransaction = next; }), true);
...
test("destructive IME selection emits once, cancel changes nothing, and confirmation requires fresh exact re-entry", () => {
  ...
  assert.deepEqual(confirmed, { kind: "REENTER_COMPOSITION", message: "批注已确认待撤下，请重新输入刚才的文字" });
  assert.deepEqual(session.beginComposition(state), { kind: "AUTHORIZED" });
  const replacement = session.inspectTransaction(state, state.tr.insertText("他").setMeta("composition", 2), { source: "composition" });
...
test("IME authorization expires when document or selection changes before re-entry", () => {
  ...
  assert.equal(session.confirmPendingAnnotationImpact(blocked.pending.token, state).kind, "REENTER_COMPOSITION");

  const moved = applyAccepted(session, state, state.tr.setSelection(TextSelection.create(state.doc, 3)));
...
test("cancelling an authorized composition restores its anchor and removes silent redo authorization", () => {
  const session = createAnnotationGuardSession({ baseAnnotationIds: ["a"] });
  let state = editorState([{ text: "我喜欢你", marks: [annotationMark("a")] }], { from: 1, to: 2 }, [history()]);
  const blocked = session.beginComposition(state);
  assert.equal(blocked.kind, "BLOCK");
  assert.equal(session.confirmPendingAnnotationImpact(blocked.pending.token, state).kind, "REENTER_COMPOSITION");
  assert.deepEqual(session.beginComposition(state), { kind: "AUTHORIZED" });

  const replacement = session.inspectTransaction(state, state.tr.insertText("他").setMeta("composition", 3), { source: "composition" });
  assert.equal(replacement.kind, "REPLACE");
