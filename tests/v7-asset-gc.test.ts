...
import {
  assertUntrackedR2Execution,
  runAssetGcCandidates,
  untrackedR2Candidates,
  type AssetGcCandidate,
  type AssetGcFailureCode,
} from "../lib/assets/gc-core.ts";
...
function dependencies(events: string[], finalReferenced = false) {
  ...
  return {
    async referenceCounts(assetId: string) {
      ...
      const referenced =
        assetId === "referenced" || (assetId === "expired" && finalReferenced && expiredChecks > 1);
      return { currentRefCount: referenced ? 1 : 0, revisionRefCount: 0, avatarRefCount: 0 };
    },
    async claim(assetId: string) {
      events.push(`claim:${assetId}`);
      return true;
    },
    async releaseClaim(assetId: string) {
      events.push(`release:${assetId}`);
    },
    ...
    async markDeleted(assetId: string) {
      events.push(`mark:${assetId}`);
    },
    async recordFailure(assetId: string, code: AssetGcFailureCode) {
      events.push(`failure:${assetId}:${code}`);
    },
  };
}
...
test("a new reference after claim wins the final GC recheck", async () => {
  ...
  const report = await runAssetGcCandidates({
    ...
    now,
    dryRun: false,
    dependencies: dependencies(events, true),
  });
...
test("V7 GC migration is additive and binding paths reject claimed assets", async () => {
  ...
  const columns = db
    ...
    .all()
    .map((row) => ({ ...row })) as Array<{ name: string; dflt_value: string | null }>;
  ...
  for (const source of [posts, saveTransaction, docx, profile])
    assert.match(source, /gcClaimedAt|gc_claimed_at/);
});
...
test("R2 inventory only reports old site-shaped objects absent from D1 and requires exact dry-run confirmation", () => {
  ...
  const found = untrackedR2Candidates(
    [
      ...
      {
        key: `${owner}/image/31112233-4455-4677-8899-aabbccddeeff-recent.png`,
        size: 14,
        uploaded: recent,
      },
  ...
  assert.deepEqual(found, [
    { assetId: orphanId, key: orphanKey, bytes: 12, reason: "UNTRACKED_R2_OBJECT" },
  ]);
