...
test("annotated save persists revision membership retirement snapshots and timestamps atomically", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      row(
        harness.sqlite,
        "SELECT title, markdown, current_revision_id, edited_at, last_activity_at FROM posts WHERE id = ?",
        POST,
      ),
      {
        title: "修改后标题",
    ...
    assert.deepEqual(
      harness.sqlite
        .prepare(
          "SELECT annotation_id FROM post_annotation_anchors WHERE post_id = ? ORDER BY annotation_id",
        )
        .all(POST)
        .map((value) => ({ ...value })),
      [{ annotation_id: B }],
    ...
    assert.deepEqual(
      row(
        harness.sqlite,
        "SELECT deleted_at, anchor_retired_at, anchor_retired_by_user_id, anchor_retired_reason FROM annotations WHERE id = ?",
        A,
      ),
      {
        deleted_at: null,
    ...
    assert.deepEqual(
      row(
        harness.sqlite,
        "SELECT original_selected_text, anchor_retired_at FROM annotations WHERE id = ?",
        B,
      ),
      {
    ...
    assert.deepEqual(
      harness.sqlite
        .prepare("SELECT annotation_id FROM revision_annotation_states WHERE revision_id = ?")
        .all(REVISION_2)
        .map((value) => ({ ...value })),
      [{ annotation_id: B }],
    );
    assert.deepEqual(
      harness.sqlite
        .prepare(
          "SELECT annotation_reply_id FROM revision_imported_reply_states WHERE revision_id = ?",
        )
        .all(REVISION_2)
        .map((value) => ({ ...value })),
...
test("a mid-batch failure rolls back every annotated save relation", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      row(
        harness.sqlite,
        "SELECT anchor_retired_at, anchor_retired_reason, original_selected_text FROM annotations WHERE id = ?",
        A,
      ),
      {
        anchor_retired_at: null,
    ...
    assert.equal(
      Number(
        harness.sqlite
          .prepare("SELECT COUNT(*) AS value FROM revision_annotation_states WHERE revision_id = ?")
          .get(REVISION_2)?.value,
      ),
      0,
    );
  } finally {
...
test("a stale CAS base aborts the complete annotated save", async () => {
  ...
  try {
    ...
    assert.equal(count(harness.sqlite, "post_annotation_anchors"), 2);
    assert.deepEqual(
      row(harness.sqlite, "SELECT title, current_revision_id FROM posts WHERE id = ?", POST),
      {
        title: "原标题",
        current_revision_id: REVISION_1,
      },
...
async function executeSave(
  ...
  await saveTransaction.commitPostSave(
    (items) => database.batch(items as [BatchItem<"sqlite">, ...BatchItem<"sqlite">[]]),
    sections,
  );
}
...
async function createHarness(failAfterStatement?: number) {
  ...
  const database = drizzle<typeof schema>(
    execute,
    async (queries) => {
      sqlite.exec("BEGIN");
      try {
...
function seed(db: DatabaseSync) {
  ...
  db.prepare(
    "INSERT INTO post_revisions (id, post_id, revision_number, kind, title, markdown, created_at, created_by_user_id, restore_source_revision_id) VALUES (?, ?, 1, 'CONTENT_EDIT', ?, ?, ?, ?, NULL)",
  ...
  db.prepare(
    "INSERT INTO import_batches (id, importer_user_id, source_filename, source_sha256, post_id, revision_id, committed_at) VALUES (?, ?, 'source.docx', ?, ?, ?, ?)",
  ).run(IMPORT_BATCH, AUTHOR, "a".repeat(64), POST, REVISION_1, ORIGINAL_TIME);
  db.prepare(
    "INSERT INTO annotations (id, post_id, author_id, content_markdown, original_selected_text, created_at, created_on_revision_id, submission_key) VALUES (?, ?, ?, '批注 A', '原文 A', ?, ?, ?)",
  ...
  db.prepare(
    "INSERT INTO annotation_replies (id, annotation_id, author_id, content_markdown, submission_key, created_at, source_type, source_author_name, source_comment_id, source_document_order, source_resolved, import_batch_id, imported_by_user_id) VALUES (?, ?, NULL, '导入回复', ?, ?, 'DOCX_IMPORT', 'Word 回复者', 'reply-b', 1, 0, ?, ?)",
  ...
  for (const id of [A, B]) {
    ...
    db.prepare(
      "INSERT INTO revision_annotation_states (revision_id, annotation_id, deleted_at, deleted_by_user_id, hidden_at, hidden_by_user_id) VALUES (?, ?, NULL, NULL, NULL, NULL)",
    ).run(REVISION_1, id);
  }
  db.prepare(
    "INSERT INTO revision_imported_reply_states (revision_id, annotation_reply_id, deleted_at, deleted_by_user_id, hidden_at, hidden_by_user_id) VALUES (?, ?, NULL, NULL, NULL, NULL)",
