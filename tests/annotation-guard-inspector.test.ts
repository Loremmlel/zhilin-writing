...
function stateFrom(source: string): EditorState {
  ...
  assert.equal(
    cursors.length + Number(Boolean(from || to)),
    1,
    "use one cursor or one selection marker pair",
  );
  ...
  const cleanPosition = (marker: Marker) =>
    marker.pos - markers.filter((candidate) => candidate.pos < marker.pos).length;
...
function markedDocument(
  ...
  const content = segments.map((segment) =>
    schema.text(segment.text, segment.marks ?? segment.annotationIds?.map(annotationMark) ?? []),
  );
...
test("allows selection-only transactions and internal insertion", () => {
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(
      selected.doc,
      selected.tr.setSelection(TextSelection.create(selected.doc, selected.selection.from)),
    ),
    { kind: "SAFE" },
  );
...
test("keeps exact-boundary insertion outside non-inclusive adjacent anchors", () => {
  ...
  assert.deepEqual(inspectAnnotationTransaction(state.doc, transaction), { kind: "SAFE" });
  assert.deepEqual(
    scanAnnotationRanges(transaction.doc).map((range) => [range.annotationId, range.text]),
    [
      ["a", "AA"],
      ["b", "BB"],
    ],
...
test("allows internal Backspace, selection deletion, and selection replacement", () => {
  ...
  const backspaceTransaction = backspace.tr.delete(
    backspace.selection.from - 1,
    backspace.selection.from,
  );
  assert.deepEqual(inspectAnnotationTransaction(backspace.doc, backspaceTransaction), {
    kind: "SAFE",
  });
  ...
  for (const transactionFor of [
    ...
    assert.deepEqual(inspectAnnotationTransaction(state.doc, transactionFor(state)), {
      kind: "SAFE",
    });
  }
});
...
test("detects both ways of destroying the protected left endpoint", () => {
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(
      outside.doc,
      outside.tr.delete(outside.selection.from, outside.selection.from + 1),
    ),
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(
      inside.doc,
      inside.tr.delete(inside.selection.from - 1, inside.selection.from),
    ),
    impact(["a"], [{ annotationId: "a", code: "LEFT_ENDPOINT_REMOVED" }]),
  );
});
...
test("detects both ways of destroying the protected right endpoint", () => {
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(
      outside.doc,
      outside.tr.delete(outside.selection.from - 1, outside.selection.from),
    ),
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(
      inside.doc,
      inside.tr.delete(inside.selection.from, inside.selection.from + 1),
    ),
    impact(["a"], [{ annotationId: "a", code: "RIGHT_ENDPOINT_REMOVED" }]),
  );
});
...
test("classifies deletion of a one-grapheme or whole anchor as removed", () => {
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(one.doc, one.tr.deleteSelection()),
    impact(["a"], [{ annotationId: "a", code: "REMOVED" }]),
  );

  const whole = stateFrom("⟦:annotation[整段文字]{#a}⟧");
  assert.deepEqual(
    inspectAnnotationTransaction(whole.doc, whole.tr.deleteSelection()),
    impact(["a"], [{ annotationId: "a", code: "REMOVED" }]),
  );
});
...
test("aggregates two destroyed annotations into one impact", () => {
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(state.doc, state.tr.insertText("replacement")),
    impact(
      ["a", "b"],
      [
...
test("allows a canonical cross-block split and join while protecting the logical outer endpoints", () => {
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(paragraph.doc, paragraph.tr.deleteSelection()),
    impact(["a"], [{ annotationId: "a", code: "REMOVED" }]),
  );

  const join = stateFrom(`第一段\n\n${CURSOR}:annotation[第二段]{#a}`);
  assert.deepEqual(inspectAnnotationTransaction(join.doc, commandTransaction(join, joinBackward)), {
    kind: "SAFE",
  });
});
...
test("allows heading conversion, list wrapping, indent, and outdent when the anchor stays legal", () => {
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(
      list.doc,
      commandTransaction(list, wrapInList(schema.nodes.bullet_list)),
    ),
    { kind: "SAFE" },
  );
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(
      indent.doc,
      commandTransaction(indent, sinkListItem(schema.nodes.list_item)),
    ),
    { kind: "SAFE" },
  );
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(
      outdent.doc,
      commandTransaction(outdent, liftListItem(schema.nodes.list_item)),
    ),
    { kind: "SAFE" },
  );
});
...
test("allows ordinary formatting across annotation boundaries", () => {
  const fixtures: Array<[string, (state: EditorState) => Transaction]> = [
    ...
    [
      ...
      (state) =>
        commandTransaction(
          state,
          toggleMark(schema.marks.link, { href: "https://example.test", title: null }),
        ),
    ],
  ];
  for (const [label, transactionFor] of fixtures) {
    ...
    assert.deepEqual(
      inspectAnnotationTransaction(state.doc, transactionFor(state)),
      { kind: "SAFE" },
      label,
    );
  }
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(
      linked.doc,
      linked.tr.removeMark(linked.selection.from, linked.selection.to, schema.marks.link),
    ),
    { kind: "SAFE" },
  );
});
...
test("classifies duplicate and malformed multi-block anchors", () => {
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(base.doc, replacementTransaction(base, duplicate)),
    impact(["a"], [{ annotationId: "a", code: "DUPLICATE" }]),
  );

  const multiBlock = parse(":annotation[A]{#a}\n\n:annotation[B]{#a}");
  assert.deepEqual(
    inspectAnnotationTransaction(base.doc, replacementTransaction(base, multiBlock)),
    { kind: "SAFE" },
  );

  const gapped = parse(":annotation[A]{#a}\n\ngap\n\n:annotation[B]{#a}");
  assert.deepEqual(
    inspectAnnotationTransaction(base.doc, replacementTransaction(base, gapped)),
...
test("classifies nested, overlapping, empty, and invalid-block anchors", () => {
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(base.doc, replacementTransaction(base, nested)),
    impact(
      ["a", "b"],
      [
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(base.doc, replacementTransaction(base, overlap)),
    impact(
      ["a", "b"],
      [
  ...
  assert.deepEqual(
    inspectAnnotationTransaction(base.doc, replacementTransaction(base, empty)),
    impact(["a"], [{ annotationId: "a", code: "EMPTY" }]),
  );

  const invalid = parse("| 内容 |\n| --- |\n| :annotation[table]{#a} |");
  assert.deepEqual(
    inspectAnnotationTransaction(base.doc, replacementTransaction(base, invalid)),
