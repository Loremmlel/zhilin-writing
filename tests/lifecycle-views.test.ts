...
test("reply tree retains only unavailable ancestors required by visible descendants", () => {
  const views = buildReplyLifecycleViews([
    { id: "root-a", authorId: "a", rootReplyId: null, replyToReplyId: null, deletedAt: new Date(1_000), hiddenAt: null },
    { id: "nested-b", authorId: "b", rootReplyId: "root-a", replyToReplyId: "root-a", deletedAt: new Date(1_100), hiddenAt: null },
    { id: "nested-c", authorId: "c", rootReplyId: "root-a", replyToReplyId: "nested-b", deletedAt: null, hiddenAt: null },
    { id: "orphan-deleted", authorId: "d", rootReplyId: null, replyToReplyId: null, deletedAt: new Date(1_200), hiddenAt: null },
  ]);

  assert.deepEqual(views.map((view) => ({ id: view.id, state: view.state, contentVisible: view.contentVisible })), [
...
test("hidden reply placeholder never reuses stored Markdown", () => {
  const [view] = buildReplyLifecycleViews([
    { id: "root", authorId: "a", rootReplyId: null, replyToReplyId: null, deletedAt: null, hiddenAt: new Date(1_000) },
    { id: "child", authorId: "b", rootReplyId: "root", replyToReplyId: "root", deletedAt: null, hiddenAt: null },
  ]);
