...
test("V7 adds a nullable per-author post creation idempotency key", async () => {
  ...
  db.prepare(
    "INSERT INTO posts (id, author_id, title, markdown, search_text, published_at, last_activity_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
  ).run("legacy", "author-a", "旧帖", "正文", "正文", 2, 2);

  await applyMigration(db, migration);
  assert.equal(
    db.prepare("SELECT creation_submission_key FROM posts WHERE id = 'legacy'").get()
      ?.creation_submission_key,
    null,
  );

  const insert = db.prepare(
    "INSERT INTO posts (id, author_id, creation_submission_key, title, markdown, search_text, published_at, last_activity_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
  );
...
test("the complete V7 migration chain upgrades a populated V6 database", async () => {
  ...
  for (const filename of files.filter((name) => name >= "0007_"))
    await applyMigration(db, filename);

  assert.deepEqual(
    {
      ...db
        .prepare("SELECT id, title, creation_submission_key FROM posts WHERE id = ?")
        .get("legacy-post"),
    },
...
test("new-post retries query the same server key before and after a unique race", async () => {
  ...
  assert.equal(
    (service.match(/findPostByCreationSubmissionKey\(authorId, submissionKey\)/g) ?? []).length,
    2,
  );
