...
test("V3 migration backfills one initial revision and preserves existing asset references", async () => {
  ...
  db.prepare("INSERT INTO posts (id, author_id, title, markdown, search_text, published_at, edited_at, last_activity_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
    .run("post-1", "user-1", "旧标题", "![旧图](/api/assets/image-a)", "旧图", 200, null, 200);
  db.prepare("INSERT INTO assets (id, owner_id, post_id, r2_key, kind, filename, mime_type, byte_size, status, created_at, bound_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'permanent', ?, ?)")
    .run("image-a", "user-1", "post-1", "image-a", "image", "a.png", "image/png", 10, 190, 200);
  db.prepare("INSERT INTO assets (id, owner_id, post_id, r2_key, kind, filename, mime_type, byte_size, status, created_at, bound_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'permanent', ?, ?)")
    .run("file-b", "user-1", "post-1", "file-b", "attachment", "b.docx", "application/docx", 20, 190, 200);
  ...
  assert.deepEqual({ ...db.prepare("SELECT current_revision_id, title, markdown, edited_at, last_activity_at FROM posts WHERE id = 'post-1'").get() }, {
  ...
  assert.deepEqual({ ...db.prepare("SELECT id, post_id, revision_number, title, markdown, created_by_user_id, restore_source_revision_id FROM post_revisions").get() }, {
  ...
  assert.deepEqual(db.prepare("SELECT asset_id, usage FROM post_asset_refs ORDER BY asset_id").all().map((row) => ({ ...row })), [
    { asset_id: "file-b", usage: "attachment" },
    { asset_id: "image-a", usage: "inline" },
  ]);
  assert.deepEqual(db.prepare("SELECT asset_id, usage FROM revision_asset_refs ORDER BY asset_id").all().map((row) => ({ ...row })), [
    { asset_id: "file-b", usage: "attachment" },
    { asset_id: "image-a", usage: "inline" },
  ]);
});
