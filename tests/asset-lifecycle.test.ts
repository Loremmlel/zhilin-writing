...
test("active current post and avatar references are member-readable", () => {
  ...
  assert.equal(
    decideAssetReadAccess(
      { ...base, avatarRefCount: 0, activeCurrentRefCount: 1 },
      { userId: "member", isAdmin: false },
    ),
    "allow",
  );
  assert.equal(
    decideAssetReadAccess(
      { ...base, avatarRefCount: 1, activeCurrentRefCount: 0 },
      { userId: "member", isAdmin: false },
    ),
    "allow",
  );
...
test("revision-only and unavailable-post assets are administrator-only even for the original owner", () => {
  ...
  for (const refs of [
    ...
    assert.equal(
      decideAssetReadAccess({ ...base, ...refs }, { userId: "owner", isAdmin: false }),
      "deny",
    );
    assert.equal(
      decideAssetReadAccess({ ...base, ...refs }, { userId: "admin", isAdmin: true }),
      "allow",
    );
  }
});
