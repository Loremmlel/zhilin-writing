...
export function AnnotationGuardDialog({
  pending,
  annotations,
  message,
  onCancel,
  onConfirm,
}: AnnotationGuardDialogProps) {
        ...
        <>
          <ul className="annotation-guard-list" aria-label="受影响的批注">
            {pending.affectedAnnotationIds.slice(0, 5).map((annotationId) => {
              ...
              const excerpt =
                pending.excerpts.find((item) => item.annotationId === annotationId)?.text ??
                "被批注文字";
              return (
                <li key={annotationId}>
                  <blockquote>{excerpt}</blockquote>
                  <span>
                    {annotation?.authorName ?? "批注作者"} · {annotation?.replyCount ?? 0} 条回复
                  </span>
                </li>
              );
            })}
          ...
          <p className="annotation-guard-local-note">
            现在只会修改本地草稿；批注讨论将在你保存正文时一并撤下。撤销或放弃草稿可完整恢复。
          </p>
          {message && (
