...
export function AnnotationSheet(props: AnnotationSheetProps) {
  const { annotation, open, onClose, onLocate } = props;
  ...
  return <ModalDialog open={open} title="正文批注" description={`选中文字：${excerpt}`} onClose={onClose} surfaceClassName="annotation-sheet-surface" backdropClassName="annotation-sheet-backdrop">
    ...
    <div className="annotation-sheet-thread">{props.readOnly === true
      ? <AnnotationReadonlyThread annotation={annotation} onLocate={onLocate} />
      : <AnnotationThread annotation={annotation} replyAction={props.replyAction} deleteAction={props.deleteAction} deleteReplyAction={props.deleteReplyAction} removeImportedAction={props.removeImportedAction} onLocate={onLocate} />}
    </div>
  </ModalDialog>;
}
