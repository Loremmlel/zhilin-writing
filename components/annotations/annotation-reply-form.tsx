...
export function AnnotationReplyForm({ action, initialSubmissionKey, label = "回复批注", focusRequest = 0, onSuccess }: {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => editorRootRef.current?.querySelector<HTMLElement>(".ProseMirror")?.focus());
