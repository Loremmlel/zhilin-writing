...
import {
  ...
  type AnnotationDeleteAction,
  type AnnotationRemoveImportedAction,
  type AnnotationReplyAction,
  type AnnotationReplyDeleteAction,
...
function AnnotationComposer({
  ...
  return (
    <ModalDialog
      open
      title="添加正文批注"
      description="批注发布后不能编辑；写错时可以删除或回复补充。"
      onClose={() => {
        if (!pending) onClose();
      }}
    >
...
export function AnnotationReadingLayout({
  postId,
  html,
  annotations,
  baseRevisionId,
  ...
  initialAnnotationReplyId?: string;
}) {
  ...
  const [deepLinkHighlight, setDeepLinkHighlight] = useState<{
    annotationId: string;
    replyId: string | null;
  } | null>(null);
  const currentSelectionContext =
    selectionContext?.saved.postId === postId &&
    selectionContext.saved.baseRevisionId === baseRevisionId
      ? selectionContext
      : null;
  const previewSelection =
    currentSelectionContext && currentSelectionContext.phase !== "hidden"
      ? currentSelectionContext.saved
      : null;

  const transitionSelectionPreview = useCallback((event: SelectionPreviewEvent) => {
    setSelectionContext((current) =>
      current ? { ...current, phase: nextSelectionPreviewPhase(current.phase, event) } : current,
    );
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    const range = validateSavedAnnotationSelection(previewSelection, {
      postId,
      baseRevisionId,
      root,
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      const anchor = bodyRef.current
        ? findAnnotationAnchorElements(bodyRef.current, initialAnnotationId)[0]
        : null;
      const annotation = annotations.find((item) => item.id === initialAnnotationId);
      if (!anchor || !annotation) return;
      const replyAvailable =
        !initialAnnotationReplyId ||
        annotation.replies.some((reply) => reply.id === initialAnnotationReplyId);
      ...
      nestedFrame = window.requestAnimationFrame(() => {
        const targetId =
          initialAnnotationReplyId && replyAvailable
            ? `annotation-reply-${initialAnnotationReplyId}`
            : layoutMode === "desktop"
              ? `annotation-card-${initialAnnotationId}`
              : null;
        const target = targetId ? document.getElementById(targetId) : null;
        target?.scrollIntoView({ block: "nearest", behavior: reducedMotion ? "auto" : "smooth" });
        target?.focus({ preventScroll: true });
        setDeepLinkHighlight({
          annotationId: initialAnnotationId,
          replyId: initialAnnotationReplyId && replyAvailable ? initialAnnotationReplyId : null,
        });
      });
    });
    return () => {
  ...
  useEffect(() => {
    ...
    if (!body) return;
    body
      .querySelectorAll(".annotation-range.is-active, .annotation-range.is-deep-linked")
      .forEach((element) => element.classList.remove("is-active", "is-deep-linked"));
    if (activeId)
      findAnnotationAnchorElements(body, activeId).forEach((element) =>
        element.classList.add("is-active"),
      );
    if (deepLinkHighlight)
      findAnnotationAnchorElements(body, deepLinkHighlight.annotationId).forEach((element) =>
        element.classList.add("is-deep-linked"),
      );
  ...
  const inspectSelection = useCallback(
    (eventTarget?: EventTarget | null) => {
      ...
      )?.closest<HTMLElement>(".annotation-range");
      if (existing?.dataset.annotationId) {
        setActiveId(existing.dataset.annotationId);
        setSheetId((current) =>
          nextAnnotationSheetState(current, {
            type: "activate",
            annotationId: existing.dataset.annotationId!,
      ...
      try {
        ...
        setSelectionContext({
          saved,
          left: Math.min(window.innerWidth - 118, Math.max(12, rect.left + rect.width / 2 - 50)),
          top: Math.max(12, rect.top - 44),
          phase: nextSelectionPreviewPhase("hidden", "capture"),
  ...
  const activateFromBody = (target: EventTarget | null, openSheet = false) => {
    ...
    if (!id) return;
    setActiveId(id);
    if (openSheet && anchor && !isNestedInteractiveTarget(target, anchor))
      setSheetId((current) =>
        nextAnnotationSheetState(current, {
          type: "activate",
          annotationId: id,
  ...
  return (
    <div
      className={`annotation-reading-layout${annotations.length === 0 ? " annotation-reading-layout--empty" : ""}`}
      data-annotation-layout={layoutMode}
      ref={layoutRef}
    >
      <div
        ref={bodyRef}
        className="markdown-body annotation-document-body"
        tabIndex={-1}
        onClick={(event) => {
          ...
          if (
            target &&
            anchor &&
            !isNestedInteractiveTarget(target, anchor) &&
            layoutMode === "compact"
          )
            event.preventDefault();
        ...
        onKeyDown={(event) => {
          const target = event.target instanceof Element ? event.target : null;
          const anchor = target?.closest<HTMLElement>(".annotation-range");
          if (target === anchor && (event.key === "Enter" || event.key === " ")) {
            event.preventDefault();
            activateFromBody(event.target, true);
          }
        }}
      ...
      <svg className="annotation-connectors" aria-hidden="true">
        {connectors.map((connector) => (
          <path
            ...
            d={connector.path}
            className={activeId === connector.annotationId ? "is-active" : ""}
          />
        ))}
      </svg>
      <aside
        ...
        aria-label={`正文批注，共 ${annotations.length} 条`}
        ...
        {layoutMode === "desktop" &&
          annotations.map((annotation) => (
            <article
              ...
              className={`annotation-card${activeId === annotation.id ? " is-active" : ""}${deepLinkHighlight?.annotationId === annotation.id && !deepLinkHighlight.replyId ? " is-deep-linked" : ""}`}
            ...
            >
              <AnnotationThread
                ...
                deleteReplyAction={deleteReplyAction}
                removeImportedAction={removeImportedAction}
                highlightReplyId={
                  deepLinkHighlight?.annotationId === annotation.id
                    ? deepLinkHighlight.replyId
                    : null
                }
                onLocate={() => {
                  ...
                  if (bodyRef.current)
                    findAnnotationAnchorElements(bodyRef.current, annotation.id)[0]?.scrollIntoView(
                      { block: "center", behavior: reducedMotion ? "auto" : "smooth" },
                    );
                }}
              />
            </article>
      ...
      <AnnotationSheet
        annotation={annotations.find((annotation) => annotation.id === sheetId) ?? null}
        open={Boolean(sheetId)}
        onClose={() =>
          setSheetId((current) => nextAnnotationSheetState(current, { type: "close" }))
        }
        replyAction={replyAction}
        deleteAction={deleteAction}
        deleteReplyAction={deleteReplyAction}
        removeImportedAction={removeImportedAction}
        highlightReplyId={
          deepLinkHighlight?.annotationId === sheetId ? deepLinkHighlight.replyId : null
        }
        highlighted={deepLinkHighlight?.annotationId === sheetId && !deepLinkHighlight.replyId}
      />
      {currentSelectionContext?.phase === "bubble" && (
        <button
          type="button"
          className="annotation-selection-menu"
          style={{ left: currentSelectionContext.left, top: currentSelectionContext.top }}
          onPointerDown={(event) => event.preventDefault()}
          onClick={() => transitionSelectionPreview("open-composer")}
        >
          添加批注
        </button>
      )}
      {currentSelectionContext &&
        ["composer", "pending", "failed"].includes(currentSelectionContext.phase) && (
          <AnnotationComposer
