...
function ReplyItem({
  ...
  return (
    <article
      className={nested ? "reply reply--nested" : "reply"}
      id={`reply-${item.reply.id}`}
      tabIndex={-1}
    >
      <div className="reply-author">
        <Avatar name={item.author.displayName} assetId={item.author.avatarAssetId} size="small" />
      </div>
      <div className="reply-body">
        <div className="reply-meta">
          ...
          {item.replyTo && (
            <span>
              {item.replyToUnavailable
                ? "回复了一条已删除或隐藏的回复"
                : `回复 ${item.replyTo.displayName}`}
            </span>
          )}
        ...
        {item.lifecycle.contentVisible ? (
          <div
            className="markdown-body markdown-body--reply"
        ...
        {item.lifecycle.contentVisible && (
          <LazyReplyForm
            action={replyActionFor(item.reply.id)}
            initialSubmissionKey={crypto.randomUUID()}
            label={`回复 ${item.author.displayName}`}
            openLabel="回复"
            compact
          />
        )}
        {item.lifecycle.contentVisible && item.reply.authorId === currentUserId && (
          <div className="delete-reply-form">
            <DeleteContentControl
              action={deleteActionFor(item.reply.id)}
              label="删除这条回复"
              title="删除这条回复？"
              description={deleteReplyConfirmation(
                item.lifecycle.visibleOtherAuthorDependentCount > 0,
              )}
            />
          </div>
        )}
...
export function ReplyList({
  ...
  items
    .filter((item) => item.reply.rootReplyId)
    .forEach((item) => {
  ...
  return (
    <div className="reply-list">
      {roots.map((root) => (
        <div className="reply-thread" key={root.reply.id}>
          <ReplyItem
            item={root}
            nested={false}
            currentUserId={currentUserId}
            replyActionFor={replyActionFor}
            deleteActionFor={deleteActionFor}
          />
          {(nested.get(root.reply.id) ?? []).map((child) => (
            <ReplyItem
              key={child.reply.id}
              item={child}
              nested
