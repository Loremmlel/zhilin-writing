...
普通内容列最大宽度 920px，编辑器 1080px。管理后台扩展至 1480px，以 220px 索引式左栏、弹性工作区和 292px 摘要栏组成；1200px 以下收起摘要栏，900px 以下左栏收进“管理导航”。后台不显示社区页脚，并占满站点 Header 下方的剩余视口；页面本身不滚动，内容表格负责双轴滚动，总览、成员、日志和摘要栏各自在面板内滚动。表头、选择列和操作列保持可见，内容预览与引用各占固定高度，行操作不换行。带批注的桌面帖子扩展至 1320px，以舒适的 720px 正文列、56px 连线沟槽和 340–400px 右栏构成；卡片按正文位置顺序错开。Annotation 组件以自身可用宽度为唯一布局来源：至少 1116px（720px 正文 + 56px gutter + 340px rail）才显示侧栏和引导线，否则使用底部 Sheet。管理员历史在宽屏采用 320px 左侧时间线和右侧预览；900px 以下堆叠，时间线自身限高滚动。所有固定弹窗保留安全边距，移动端转为单列操作。
